...
use crate::{
    config::{AlpacaDataClientConfig, AlpacaExecutionClientConfig, AlpacaDataFeed},
...
fn extract_alpaca_exec_config(
    ...
    match config.extract::<AlpacaExecutionClientConfig>(py) {
        Ok(c) => Ok(Box::new(c)),
        Err(e) => Err(to_pyvalue_err(format!(
            "Failed to extract AlpacaExecutionClientConfig: {e}"
        ))),
    }
}
