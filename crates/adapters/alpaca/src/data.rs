...
use crate::config::{AlpacaDataClientConfig, AlpacaDataFeed};

const ALPACA_PRICE_PRECISION: u8 = 4;
const ALPACA_SIZE_PRECISION: u8 = 0;

enum AlpacaCommand {
    SubscribeQuotes(Vec<String>),
    SubscribeTrades(Vec<String>),
    UnsubscribeQuotes(Vec<String>),
    UnsubscribeTrades(Vec<String>),
}

/// Data client for Alpaca.
pub struct AlpacaDataClient {
    client_id: ClientId,
    config: AlpacaDataClientConfig,
    data_sender: mpsc::UnboundedSender<DataEvent>,
    cmd_sender: Option<mpsc::UnboundedSender<AlpacaCommand>>,
...
impl AlpacaDataClient {
    ...
    pub fn new(client_id: ClientId, config: AlpacaDataClientConfig) -> Self {
    ...
    fn ws_url(&self) -> &'static str {
        match self.config.feed {
            AlpacaDataFeed::Iex => "wss://stream.data.alpaca.markets/v2/iex",
            AlpacaDataFeed::Sip => "wss://stream.data.alpaca.markets/v2/sip",
        }
    }
}
...
impl DataClient for AlpacaDataClient {
    ...
    fn start(&mut self) -> anyhow::Result<()> {
        log::info!("Starting Alpaca data client...");
        Ok(())
    }

    fn stop(&mut self) -> anyhow::Result<()> {
        log::info!("Stopping Alpaca data client...");
    ...
    async fn connect(&mut self) -> anyhow::Result<()> {
        ...
        if let Some(Ok(Message::Text(text))) = read.next().await {
            ...
            if msgs[0]["T"] != "success" || msgs[0]["msg"] != "connected" {
                return Err(anyhow::anyhow!("Unexpected Alpaca connection message: {}", text));
            }
        }
        ...
        log::info!("Alpaca data client connected and authenticated.");

        let (cmd_tx, mut cmd_rx) = mpsc::unbounded_channel::<AlpacaCommand>();
        self.cmd_sender = Some(cmd_tx);

        // 3. Spawn message handling loop
                ...
                tokio::select! {
                    ...
                    msg = read.next() => {
                        match msg {
                            ...
                            Some(Err(e)) => {
                                log::error!("Alpaca WS read error: {}", e);
                                break;
                            }
                            None => {
                                log::warn!("Alpaca WS stream ended.");
                                break;
                            }
                            _ => {}
                        }
                    ...
                    cmd = cmd_rx.recv() => {
                        if let Some(command) = cmd {
                            let msg = match command {
                                AlpacaCommand::SubscribeQuotes(symbols) => json!({
                                    "action": "subscribe",
                                    "quotes": symbols,
                                }),
                                AlpacaCommand::SubscribeTrades(symbols) => json!({
                                    "action": "subscribe",
                                    "trades": symbols,
                                }),
                                AlpacaCommand::UnsubscribeQuotes(symbols) => json!({
                                    "action": "unsubscribe",
                                    "quotes": symbols,
                                }),
                                AlpacaCommand::UnsubscribeTrades(symbols) => json!({
                                    "action": "unsubscribe",
                                    "trades": symbols,
                                }),
                            };
                            if let Err(e) = write.send(Message::Text(msg.to_string().into())).await {
                                log::error!("Failed to send Alpaca subscription: {}", e);
                                break;
                            }
                        } else {
                            break;
    ...
    fn subscribe_quotes(&mut self, cmd: SubscribeQuotes) -> anyhow::Result<()> {
        ...
        if let Some(tx) = &self.cmd_sender {
            tx.send(AlpacaCommand::SubscribeQuotes(vec![symbol]))?;
        }
        Ok(())
    }
    ...
    fn subscribe_trades(&mut self, cmd: SubscribeTrades) -> anyhow::Result<()> {
        let symbol = cmd.instrument_id.symbol.to_string();
        if let Some(tx) = &self.cmd_sender {
            tx.send(AlpacaCommand::SubscribeTrades(vec![symbol]))?;
        }
        Ok(())
    }
    ...
    fn unsubscribe_quotes(&mut self, cmd: &UnsubscribeQuotes) -> anyhow::Result<()> {
        let symbol = cmd.instrument_id.symbol.to_string();
        if let Some(tx) = &self.cmd_sender {
            tx.send(AlpacaCommand::UnsubscribeQuotes(vec![symbol]))?;
        }
        Ok(())
    }
    ...
    fn unsubscribe_trades(&mut self, cmd: &UnsubscribeTrades) -> anyhow::Result<()> {
        let symbol = cmd.instrument_id.symbol.to_string();
        if let Some(tx) = &self.cmd_sender {
            tx.send(AlpacaCommand::UnsubscribeTrades(vec![symbol]))?;
        }
        Ok(())
    }
}
...
fn parse_quote(msg: &Value, venue: Venue) -> Option<QuoteTick> {
    ...
    let bid_size = msg["bs"].as_f64()?;
    let ask_size = msg["as"].as_f64()?;
    ...
    Some(QuoteTick {
        instrument_id,
        bid_price: Price::new(bid_price, ALPACA_PRICE_PRECISION),
        ask_price: Price::new(ask_price, ALPACA_PRICE_PRECISION),
        bid_size: Quantity::new(bid_size, ALPACA_SIZE_PRECISION),
        ask_size: Quantity::new(ask_size, ALPACA_SIZE_PRECISION),
        ts_event: UnixNanos::new(ts_event as u64),
...
fn parse_trade(msg: &Value, venue: Venue) -> Option<TradeTick> {
    ...
    Some(TradeTick {
        instrument_id,
        price: Price::new(price, ALPACA_PRICE_PRECISION),
        size: Quantity::new(size, ALPACA_SIZE_PRECISION),
        ts_event: UnixNanos::new(ts_event as u64),
        ts_init: UnixNanos::new(Utc::now().timestamp_nanos_opt().unwrap_or(0) as u64),
        ..Default::default()
    })
